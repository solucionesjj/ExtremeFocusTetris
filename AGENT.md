...
- **Hallazgo real de infraestructura de testing, resuelto en la Fase 9:** un widget test que hace `tester.tap()` sobre cualquier `SwitchListTile` de `SettingsScreen` (incluso uno preexistente como "Sound", no solo los nuevos de esta fase) y luego deja que `setUpHiveForTesting()`'s `tearDown()` corra, **colgaba indefinidamente en `Hive.box(...).clear()`** — un `.put()` disparado por `SettingsController._persist()` (fire-and-forget, sin `await`) seguía en vuelo bajo la zona `FakeAsync` del test, y llamar `.clear()` sobre la misma caja mientras ese `.put()` no había resuelto generaba un deadlock real de Hive (confirmado con prints de diagnóstico: `TEARDOWN: start clear` imprimía, `TEARDOWN: settings cleared` nunca llegaba). **No era un bug de la Fase 8** — ningún test anterior había tocado un switch de Settings, por eso nunca se manifestó. **Solución encontrada en la Fase 9** (ver `test/widget/settings_screen_test.dart` y el doc-comment de `setUpHiveForTesting` en `test/test_helpers/hive_test_setup.dart`): envolver el tap + pump + una espera real corta en **un solo** `tester.runAsync(() async {...})` — `runAsync` pausa `FakeAsync` y deja correr el event loop real hasta que su callback resuelve, así que la escritura de Hive sí termina antes de que retorne el bloque (y por lo tanto antes de que termine el test). Un `tester.tap()`/`pump()` sueltos, sin `runAsync`, no alcanzan: hasta un delay real de 100ms *fuera* de `runAsync` no evitó el cuelgue en las pruebas de esta sesión.
...
**Fase 9 (Testing integral) completada, con verificación en vivo real en el emulador Android.** El bloqueo de la Fase 3 quedó resuelto esta fase (ver la nota de "Actualización (Fase 8...)" más arriba): el AVD arrancó con input táctil funcionando, lo que permitió construir e instalar la app real (`flutter run`/`flutter test integration_test/`) y usarla de punta a punta por primera vez desde la Fase 2.

- **Hive/`tearDown` (el hallazgo pendiente de la Fase 8) resuelto:** el fix es envolver el tap + `pump()` + una espera real corta en **un solo** `tester.runAsync(() async {...})` — no basta con un delay real *fuera* de `runAsync`. Ver el doc-comment de `setUpHiveForTesting` (`test/test_helpers/hive_test_setup.dart`) y `_tapAndLetHiveWriteSettle` en `test/widget/settings_screen_test.dart` para el patrón exacto.
- **Widget tests nuevos:** `test/widget/game_screen_hud_test.dart` (HUD Clásico muestra nivel/líneas/Hold/next-queue de 3; Concentración los oculta y next-queue baja a 1, timer se mantiene), `test/widget/hold_widget_test.dart` (slot vacío transparente, opacidad completa vs. atenuada), `test/widget/touch_controls_test.dart` (mantener el botón izquierdo dispara DAS 170ms luego ARR 30ms — verificado comparando `origin.col` antes/después; un tap simple mueve exactamente 1 columna; rotar cambia `rotation` salvo para la pieza O, que es un no-op por SRS; hold intercambia pieza; hard drop sube el score), y `test/widget/settings_screen_test.dart` (los 3 toggles nuevos de accesibilidad cambian y persisten entre pumps, el slider de escala de texto respeta 0.85–1.3, y un test de regresión específico para el bug del "Tema" de la Fase 8).
- **`integration_test/`** (paquete `integration_test` agregado a `dev_dependencies`, sin conflictos de versión): dos archivos, **cada uno con un solo `testWidgets`** — no un único archivo con dos tests. Esto no es solo estilo: un primer intento con ambos escenarios en un archivo produjo contaminación real entre tests (un `AudioPlayer` de un test aparecía ya "disposed" en el siguiente, pese a que cada `testWidgets` debería partir de un árbol de widgets fresco) — separar en archivos distintos hace que cada uno sea su propio lanzamiento de la app en el dispositivo, evitando esa clase de bug por completo.
  - `full_flow_test.dart`: Home → Play → mover/rotar/hard-drop → pausar (guarda sesión) → reanudar → pausar de nuevo → Exit to menu → de vuelta en Home. Corre real en el emulador.
  - `resume_test.dart`: juega un poco, pausa+sale (guarda sesión), y verifica la persistencia **contra el repositorio directamente** (`GameRepository.hasSavedSession()`/`loadSession()`), no a través de la etiqueta "Continuar" del botón de Home. Se abandonó deliberadamente la verificación a nivel de UI del botón tras agotar los arreglos razonables — ver el comentario extenso al inicio de ese archivo para las dos causas de fondo encontradas (conflicto de `audioplayers` con `pumpWidget(SizedBox.shrink())` al simular un "kill" del proceso, y que `HomeScreen` lee `hasSavedSession()` una sola vez de forma no reactiva en `build()`, sin que ni un re-pump no-const ni `GoRouter.refresh()` lograran forzar de forma fiable un rebuild fresco en este harness). La persistencia real ante un reinicio de proceso genuino ya estaba cubierta por el round-trip de `repositories_test.dart` y por la verificación en vivo de la Fase 8 (reinicio real de `flutter run`).
- **Golden tests** (`test/golden/`, usando el `matchesGoldenFile` nativo de `flutter_test` — **no** se agregó `golden_toolkit` como dependencia, para no repetir el patrón de conflictos de versión ya visto con `hive_generator`): tablero en estado fijo y conocido (`Board.fromCellList` + `StartNewGame.call(Random(42))` para que la pieza activa/next-queue sean deterministas) en paleta estándar/daltónica/alto-contraste; `NextQueueWidget`/`HoldWidget` en Clásico/Concentración/daltónico (deterministas por props explícitos, a diferencia de un `GameScreen` real cuyo 7-bag usa `Random()` sin semilla); `HomeScreen` en tema claro/oscuro. **Alcance recortado a propósito:** no hay golden de un `GameScreen` en vivo ni de la pantalla de Game Over — el 7-bag no determinista de `GameController` haría que esas capturas cambiaran de pixel en cada corrida, lo cual no es un golden útil; el layout HUD Clásico/Concentración que sí importa para spec.md §21 ya queda cubierto por los widgets deterministas de arriba. Regenerar con `flutter test test/golden/ --update-goldens` si cambia intencionalmente el diseño visual.
- **Checklist QA manual de spec.md §21 (ejecutado en el emulador real):**
  - [x] Abre e inicia partida en <5s — confirmado (build/instalación e inicio casi inmediatos).
  - [ ] Sin jank perceptible en ninguna pantalla — **no verificado rigurosamente** (requeriría perfilado con DevTools, sin hacer esta fase; solo observación informal sin stutter visible). Candidato explícito para la Fase 10 (Optimización de rendimiento).
  - [ ] El loop tic-toc no corta tras 30+ min — **no verificado** (requiere una prueba de resistencia larga, fuera de alcance de esta sesión).
  - [x] Los toggles de Settings persisten tras cerrar/reabrir — confirmado (reinicio real de `flutter run` en la Fase 8, más el test de la Fase 9).
  - [x] "Continuar" restaura exactamente el estado de la última partida — confirmado (`resume_test.dart` + round-trip de `repositories_test.dart`).
  - [x] Concentración reduce visiblemente el HUD vs. Clásico — confirmado (capturas en vivo + `game_screen_hud_test.dart`).
  - [x] Los 7 tetrominós son distinguibles en modo daltónico — confirmado (captura en vivo con paleta Okabe-Ito + texturas, más tests unitarios).
  - [x] El texto respeta el escalado de accesibilidad sin romper el layout — confirmado en vivo a 1.3× en Settings (el label "System"/"Sistema" del `SegmentedButton` envuelve a 2 líneas con gracia, sin overlap) y en el HUD del juego (sin overlap ni recorte).
  - [x] La app funciona en modo avión — confirmado en vivo (ícono de wifi desaparece de la barra de estado, la partida sigue jugándose sin degradación).
  - [x] No se solicita ningún permiso no declarado — confirmado por inspección: `AndroidManifest.xml` no declara ni un solo `<uses-permission>`.

**Pendiente aún:** `core/error`/`core/utils`, y todas las demás carpetas bajo `lib/features/` — se crean/amplían cuando su fase respectiva lo pida. La siguiente fase del roadmap es la **Fase 10** (Optimización de rendimiento y memoria: profiling con DevTools, auditoría de `ParticlePool`/`AudioPlayer` pooling, jank residual, tamaño de APK/AAB) — que también debería cerrar los dos ítems del checklist QA marcados como no verificados arriba.
