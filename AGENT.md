...
**Pendiente aún:** `core/error`/`core/utils`, y todas las demás carpetas bajo `lib/features/` (Settings/Statistics no tienen todavía la separación completa de casos de uso que sí tiene `game`, ya que su alcance fue deliberadamente mínimo) — se crean/amplían cuando su fase respectiva lo pida. La siguiente fase del roadmap es la **Fase 6** (Modo Concentración), aunque sigue pendiente resolver el bloqueo de emulador (nota de la Fase 3) antes de dar por buena la app en un dispositivo real.
