...
**Fase 7 (Pulido visual) completada en código.** No se pudo verificar en emulador/dispositivo (mismo bloqueo de entorno de la Fase 3/4/5/6). Implementa spec.md §7/§16/§18 (partículas, flash+shake de línea, highlight diagonal, glow de borde, scale pulses, y el popup/glow de combo diferido de la Fase 6).
...
**Pendiente aún:** `core/error`/`core/utils`, y todas las demás carpetas bajo `lib/features/` — se crean/amplían cuando su fase respectiva lo pida. La siguiente fase del roadmap es la **Fase 8** (Accesibilidad), aunque sigue pendiente resolver el bloqueo de emulador (nota de la Fase 3) antes de dar por buena la app en un dispositivo real.
