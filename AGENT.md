...
**Pendiente aún:** `core/error`/`core/utils`, la capa `data` de `features/game` (Fase 5), Hive para settings/estadísticas/sesión (Fase 5), y todas las demás carpetas bajo `lib/features/` — se crean cuando su fase respectiva empieza, no antes. La siguiente fase del roadmap es la **Fase 5** (persistencia), aunque conviene resolver primero el bloqueo de emulador (nota de la Fase 3) antes de dar por buena la app en un dispositivo real.
