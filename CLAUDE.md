...
Roadmap **Phases 0, 1, 2 and 3 are done in code**. Phase 0: the Flutter project is scaffolded (Android only, org `com.extremefocus`), theme (light/dark), Riverpod wiring, and the l10n skeleton (es/en) are in place. Phase 1: `lib/features/game/domain/` is a complete, pure-Dart game engine (SRS rotation + wall kicks, 7-bag, board/line-clear, T-Spin detection, scoring, level curve, hold, hard drop, the lock→resolve→spawn pipeline). Phase 2: `lib/features/game/presentation/` renders it — a `CustomPainter` board (with ghost piece), a `GameController` Riverpod notifier driving gravity/lock-delay off a widget-owned `Ticker`, next-queue/hold widgets, and touch controls (DAS/ARR on left-right, held soft drop, single-tap rotate/hold/hard-drop). Phase 3: `AudioService`/`HapticService` (spec.md section 6) are wired to every game event via a `LockResult` (`{state, outcome}`) that `LockActivePiece.call`/`HardDrop.call` now return instead of plain `GameState` — check that signature before touching either use case. All four phases pass `flutter analyze`/`flutter test` (69 tests). Phases 0–2 were additionally verified live on an Android emulator; **Phase 3 was not** — touch input on the emulator broke at the system level (not our code: even tapping launcher icons stopped working, survived an `adb`/app/full cold-boot-without-snapshot restart) partway through that session. `AGENT.md`'s "Estado del repositorio" has full detail, including two non-obvious things: domain entities are hand-written immutable classes, not `freezed` (Phase 1), and starting a game from a widget's `initState` must be deferred (e.g. `Future.microtask`) or Riverpod throws (hit and fixed in Phase 2). The bundled `.ogg` audio assets are synthesized placeholder tones for wiring purposes, not final sound design. Everything under other features (`home`, `settings`, `statistics`, `splash`, `game_over`, `about`), `lib/features/game/data/`, and `core/routing` doesn't exist yet — created feature-by-feature as their roadmap phase starts. Reaching the Game screen today is via a temporary "Jugar (debug)" button on the Home placeholder — real navigation is Phase 4's `go_router` work. Before starting Phase 4, get a live device/emulator confirmation that Phase 3's audio/haptics actually work, since that couldn't be verified this session.
