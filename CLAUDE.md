...
Roadmap **Phases 0 and 1 are done**. Phase 0: the Flutter project is scaffolded (Android only, org `com.extremefocus`), theme (light/dark), Riverpod wiring, and the l10n skeleton (es/en) are in place, and a debug build has been confirmed running on an Android emulator. Phase 1: `lib/features/game/domain/` is a complete, pure-Dart game engine (SRS rotation + wall kicks, 7-bag, board/line-clear, T-Spin detection, scoring, level curve, hold, hard drop, the lock→resolve→spawn pipeline) backed by 65 unit tests under `test/unit/game/`. `flutter analyze`/`flutter test` are clean. `AGENT.md`'s "Estado del repositorio" section has the exact file-by-file detail, including one deliberate deviation from the original plan (domain entities are hand-written immutable classes, not `freezed` — see that section for why). Everything under `lib/features/game/presentation|data/`, other features, and `core/routing`/`core/services` doesn't exist yet — those get created feature-by-feature as their roadmap phase starts (next up: **Phase 2**, rendering and input).
