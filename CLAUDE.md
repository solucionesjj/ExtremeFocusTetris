...
- **`SwitchListTile`/`Slider` interaction in widget tests can deadlock** — tapping any Settings toggle triggers `SettingsController._persist()`, a fire-and-forget Hive `box.put()`. If a test's `tearDown()` then calls `box.clear()` on that same box while the `put()` is still in flight, Hive deadlocks for real (confirmed by bisecting with print markers: `tearDown` prints `start clear` and then never returns). This isn't new to Phase 8 — no earlier test had ever tapped a Settings switch. Until this is fixed (candidate for Phase 9), avoid `tester.tap()` on `SettingsScreen` switches/sliders in widget tests.
...
Everything under `core/error`/`core/utils`, and the rest of each feature's folder structure beyond what Settings/Statistics/Game needed so far, doesn't exist yet — created when its roadmap phase needs it. Next up: **Phase 9** (Testing integral).
